...
<aside id="sidebar" class="hidden md:block fixed top-0 left-0 w-64 h-full bg-gradient-to-b from-[#66D2CE] to-[#4FC6B3] text-white font-semibold p-5 z-40 shadow-xl transition-all">
  <div class="flex flex-col items-center mb-8 gap-4">
    <a href="<?= site_url('dashboard'); ?>" class="flex flex-col items-center group">
      <div class="flex items-center space-x-2 transition-transform duration-300 transform group-hover:-translate-y-1 cursor-pointer mb-2">
  ...
  <nav class="space-y-2">
    ...
    <?php elseif ($role === 'admin'): ?>
      ...
      <!-- Peminjaman (admin) -->
      <div x-data="{ openPeminjaman: <?= $is_in(['adminpeminjaman','pengembalian']) ? 'true' : 'false' ?> }">
      ...
      <a href="<?= site_url('admin/user'); ?>"
         class="flex items-center gap-3 px-4 py-2 rounded-xl mt-2 transition-colors duration-200 <?= $current === 'user' ? 'bg-white/90 text-teal-700 shadow-md font-semibold' : 'hover:bg-white/20' ?>">
  ...
  <nav class="space-y-2 text-white">
    ...
    <?php elseif ($role === 'admin'): ?>
      ...
      <div x-data="{ open: <?= $is_in(['adminpeminjaman','pengembalian']) ? 'true' : 'false' ?> }">
        <button @click="open = !open" class="w-full text-left px-4 py-2 rounded-lg <?= $is_in(['adminpeminjaman','pengembalian']) ? 'bg-white/90 text-teal-700 font-semibold' : 'hover:bg-white/40' ?>">Peminjaman</button>
        <div x-show="open" x-transition class="ml-4 mt-2 space-y-1">
      ...
      <a href="<?= site_url('admin/user'); ?>" class="block px-4 py-2 rounded-lg <?= $current === 'user' ? 'bg-white/90 text-teal-700 font-semibold' : 'hover:bg-white/40' ?>">Manajemen User</a>
      <a href="<?= site_url('profile_setting'); ?>" class="block px-4 py-2 rounded-lg <?= $current === 'profile_setting' ? 'bg-white/90 text-teal-700 font-semibold' : 'hover:bg-white/40' ?>">Setting</a>
      <a href="<?= site_url('auth/logout'); ?>" class="block px-4 py-2 mt-4 rounded-lg bg-red/60 hover:bg-red-300 text-red-600 font-semibold">Log Out</a>
    <?php endif; ?>
  </nav>
