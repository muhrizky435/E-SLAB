<div class="p-2 space-y-6">
  ...
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
    <div class="flex flex-col mb-4">
      <h1 class="text-3xl font-bold text-gray-800"><i class="fa-solid fa-flask-vial mr-2 text-teal-500"></i>Katalog Bahan</h1>
      <p class="text-gray-500 text-sm mt-1 italic">“Temukan dan kelola Bahan kesehatan untuk Keperawatan dan Kebidanan dengan mudah dan cepat.”</p>
    </div>
  </div>
  ...
  <div class="overflow-x-auto bg-white rounded-2xl shadow-lg border border-gray-100">
    <table class="min-w-full text-sm text-gray-700">
      ...
      <tbody id="tableBody">
        <?= $this->load->view('admin/katalog_bahan/_rows', ['barang' => $barang, 'offset' => ($this->input->get('per_page') ?? 0)], true); ?>
      </tbody>
    </table>
  </div>
  ...
  <div id="confirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-40 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl shadow-lg p-6 w-full max-w-sm text-center animate-fade-in">
      ...
      <h2 class="text-lg font-semibold text-gray-800 mb-2">Yakin ingin menghapus bahan ini?</h2>
...
<script>
  ...
  function updateTable(keyword, kategori = '', page = 0) {
    ...
    fetch(`<?= site_url('admin/katalog_bahan/live_search'); ?>?keyword=${encodeURIComponent(keyword)}&kategori=${kategori}&page=${page}`)
      .then(r => r.json())
      .then(data => {
