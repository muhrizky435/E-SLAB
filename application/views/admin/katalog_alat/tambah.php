<div class="p-4 flex justify-center items-center min-h-screen">
  <div class="bg-white p-4 rounded-2xl shadow-xl w-full max-w-6xl transition-all duration-300 hover:shadow-2xl">
    ...
    <p class="text-gray-500 text-center text-sm mb-6 italic">Menambah inventaris alat kesehatan untuk Keperawatan dan Kebidanan dengan efisien dan akurat</p>

    <form action="<?= site_url('admin/katalog_alat/simpan'); ?>" method="post" class="space-y-5">
      <!-- Nama Alat -->
      <div>
        <label class="block text-gray-700 font-semibold mb-1">
          <i class="fa-solid fa-toolbox mr-1 text-teal-500"></i> Nama Alat
        </label>
      ...
      <div>
        ...
        <textarea name="deskripsi"
          class="w-full border border-teal-300 rounded-xl p-3 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition duration-200"
          rows="3" placeholder="Tulis deskripsi alat (opsional)"></textarea>
      </div>
  ...
  <?php if ($this->session->flashdata('modal_message')): ?>
    <div id="resultModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl shadow-lg p-6 w-full max-w-sm text-center animate-fade-in">
        ...
        <?php endif; ?>
        <p class="text-gray-600 mb-2 mt-2"><?= $this->session->flashdata('modal_message'); ?></p>
        <button onclick="closeModal('<?= site_url('admin/katalog_alat'); ?>')"
          class="bg-gradient-to-r from-teal-600 to-teal-400 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition">
          Tutup
        </button>
      </div>
    </div>
